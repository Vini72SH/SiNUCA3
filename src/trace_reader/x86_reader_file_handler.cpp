...
sinuca::traceReader::StaticTraceFile::StaticTraceFile(std::string folderPath, std::string img) {
...
void sinuca::traceReader::StaticTraceFile::ReadNextPackage(
    ...
    this->GetInsName(&info->staticInfo, data);
...
sinuca::traceReader::DynamicTraceFile::DynamicTraceFile(std::string folderPath, std::string img, THREADID tid)
    : TraceFileReader(FormatPathTidIn(folderPath, "dynamic", img, tid)) {
    this->bufActiveSize = (unsigned int)(BUFFER_SIZE / sizeof(BBLID)) * sizeof(BBLID);
...
sinuca::traceReader::MemoryTraceFile::MemoryTraceFile(std::string folderPath, std::string img, THREADID tid)
    : TraceFileReader(FormatPathTidIn(folderPath, "memory", img, tid)) {
    this->RetrieveLenBytes((void*)this->bufActiveSize, sizeof(this->tf.offset));
    this->RetrieveBuffer();
}
...
int sinuca::traceReader::MemoryTraceFile::ReadNextMemAccess(
    ...
    if (insInfo->staticInfo.isNonStdMemOp) {
        dynInfo->numReadings = *(unsigned short *)(this->GetData(SIZE_NUM_MEM_R_W));
        dynInfo->numWritings = *(unsigned short *)(this->GetData(SIZE_NUM_MEM_R_W));
    } else {
...
bool GetBitBool(unsigned char byte) {
    return (byte == 1);
}
...
void sinuca::traceReader::StaticTraceFile::GetInsName(sinuca::StaticInstructionInfo *info, struct DataINS *data) {
    strncpy(info->opcodeAssembly, data->name, MAX_INSTRUCTION_NAME_LENGTH);
}

void sinuca::traceReader::StaticTraceFile::GetFlagValues(InstructionInfo *info, struct DataINS *data) {
...
void sinuca::traceReader::StaticTraceFile::GetBranchFields(sinuca::StaticInstructionInfo *info, struct DataINS *data) {
...
void sinuca::traceReader::StaticTraceFile::GetReadRegs(sinuca::StaticInstructionInfo *info, struct DataINS *data) {
    info->numReadRegs = data->numReadRegs;
    memcpy(info->readRegs, data->readRegs,
           data->numReadRegs * sizeof(*data->readRegs));
}

void sinuca::traceReader::StaticTraceFile::GetWriteRegs(sinuca::StaticInstructionInfo *info, struct DataINS *data) {
