...
static volatile INT32 suspendResumeCount = 0;

// The pindb script performs 20 step commands, so we only need to test
// stop/resume during that phase.  Once the debugger issues 'c' the
// application runs freely; continuing to call PIN_StopApplicationThreads
// on every instruction would be extremely slow.
static const INT32 MAX_SUSPEND_RESUME = 25;
...
static VOID SuspendResume(THREADID tid)
{
    ...
    // Stop testing after enough iterations (covers the step phase).
    if (suspendResumeCount >= MAX_SUSPEND_RESUME) return;
